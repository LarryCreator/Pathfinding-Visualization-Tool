...
function calculatee_g_cost(current, el) {
  //this calculates the gcost
  el.g_cost = current.g_cost + 10;
}
function calculatee_h_cost(el, goal) {
  //this calculates the heuristic
  const position = getOffset(el.node);
  const positiong = getOffset(goal.node);
  el.h_cost = Math.sqrt(
    Math.abs(position.x - positiong.x) ** 2 +
      Math.abs(position.y - positiong.y) ** 2
  );
}
  ...
  if (!is_running) {
    ...
    var k = setInterval(function () {
      if (open_list.length > 0) {
        ...
        closed_list[0].node.classList.remove("searching");
        ...
        for (i = 0; i < new_board.length; i++) {
          ...
          if (
            ...
            calculatee_g_cost(current, new_board[i]);
            calculatee_h_cost(new_board[i], goal_node);
